...
CREATE TABLE ` + quoteIdentifier(stagingTable) + ` (
	LIKE proxy_statistics INCLUDING DEFAULTS INCLUDING GENERATED INCLUDING STORAGE INCLUDING COMMENTS,
...
`
	...
	if err := rebindProxyStatisticsIdentity(tx, stagingTable); err != nil {
		return err
	}
...
func rebindProxyStatisticsIdentity(db *gorm.DB, table string) error {
	if db == nil {
		return fmt.Errorf("nil database connection")
	}

	qualified := quoteIdentifier(table)
	if err := db.Exec(`ALTER TABLE ` + qualified + ` ALTER COLUMN id DROP DEFAULT`).Error; err != nil {
		return err
	}
	if err := db.Exec(`ALTER TABLE ` + qualified + ` ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY`).Error; err != nil {
		return err
	}
